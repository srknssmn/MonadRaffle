...
import withdrawBalance from '../../controllers/withdrawBalance';

function PastRaffle({raffleAddress, nftAddress, nftId, ticketCost, maxTicket, currentTicket, raffleStatus, nftName, nftURI}) {

    const {withdraw} = withdrawBalance();

    const handleWithdraw = () => {
        withdraw(raffleAddress);
    }
        ...
        <div className='items-center justify-center flex'>
            <div className='flex flex-col shadow-md rounded-b-lg'>
                ...
                <div className='flex flex-row items-center justify-center py-2 px-2 space-x-1'>
                    <button onClick={handleWithdraw} className='text-sm uppercase py-2 px-2 bg-green-800 text-gray-200 rounded-xl hover:bg-green-400 hover:text-gray-800 transition duration-700'>Withdraw Balance</button>
                    <Link to={`/raffles/${raffleAddress}`} className='text-sm uppercase py-2 px-2 bg-gray-800 text-gray-200 rounded-xl hover:scale-90 transition duration-700'>
                        Enter Raffle
                    </Link>
                </div>
            </div>
